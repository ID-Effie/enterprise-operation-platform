<template>
  <PageContainer title="用户管理" description="用于承载后台用户、角色归属和账号状态维护。">
    <template #actions>
      <button v-permission="'user:create'" type="button" class="primary-link" @click="openCreate">
        新增用户
      </button>
      <button v-permission="'user:assign-role'" type="button" class="secondary-link">
        分配角色
      </button>
    </template>
    ...
    <section class="query-panel" aria-label="用户查询区">
      <label>
        用户名称
        <input v-model="query.username" type="text" placeholder="请输入用户名称" />
      </label>
      <label>
        账号状态
      ...
      <button type="button" class="primary-link" @click="loadUserList">查询</button>
    ...
    <section class="table-placeholder" aria-label="用户表格区">
      ...
      <div v-else-if="errorMessage" class="table-state table-error">
        ...
        <button type="button" class="table-action" @click="loadUserList">重试</button>
      </div>
      ...
      <div v-else-if="users.length > 0" class="table-body">
        <div v-for="user in users" :key="user.id" class="table-row">
          ...
          <span>
            <StatusTag :text="userStatusText[user.status]" :color="userStatusColor[user.status]" />
          </span>
          <span>
            <button
    ...
    <div v-if="visible" class="modal-mask">
      <div class="modal-panel">
        ...
        <p v-if="current">当前用户：{{ current.nickname ?? current.username }}</p>
        <p v-else>当前为新增用户</p>

        <button type="button" class="secondary-link" @click="close">关闭</button>
      </div>
    </div>
  </PageContainer>
</template>
...
const { visible, mode, current, openCreate, openEdit, close } = useModal<UserInfo>()
